...
export function Filters({
  ...
  return (
    <Card
      ...
      aria-busy={pending}
      className={embedded ? "gap-5 border-0 bg-transparent p-0 shadow-none" : "mb-8 gap-5 p-5 shadow-none sm:p-6"}
    >
      ...
      >
        ...
        <div className="relative isolate">
          <Search className="pointer-events-none absolute top-3 left-3 z-10 size-4 text-muted-foreground" />
          <Input
            id="exercise-search"
            name="q"
            ...
            onChange={() => {
              clearTimeout(timer.current);
              timer.current = setTimeout(search, 300);
            }}
          />
        </div>
        {filterKeys.flatMap((key) =>
          selected[key].map((value) => <input key={`${key}-${value}`} type="hidden" name={key} value={value} />),
        )}
      ...
      <div className="space-y-4">
        {filterKeys.map((key) => (
          <div key={key} role="group" aria-label={t(key)} className="flex min-w-0 flex-col gap-2 sm:flex-row sm:gap-4">
            ...
            <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible sm:pb-0">
              {facets[key].map((value) => {
                ...
                const next = {
                  ...selected,
                  page: 1,
                  [key]: active ? selected[key].filter((item) => item !== value) : [...selected[key], value],
                };
                return (
                  <Button
                    key={value}
                    asChild
                    variant={active ? "lightprimary" : "ghost"}
                    className={`${tagClass} ${active ? "border-primary/30" : "border-border text-muted-foreground"}`}
                  >
                    <Link
                      prefetch={false}
                      href={queryHref(path, next)}
                      aria-current={active ? "true" : undefined}
                      onClick={(event) => selectTag(event, next)}
                    >
        ...
        <div
          role="group"
          aria-label={t("sort")}
          className="flex flex-wrap items-center gap-2 border-t border-border pt-4 sm:gap-4"
        >
          ...
          {(["name-asc", "name-desc"] as const).map((sort) => {
            ...
            return (
              <Button
                key={sort}
                asChild
                variant={selected.sort === sort ? "lightprimary" : "ghost"}
                className={`${tagClass} ${selected.sort === sort ? "border-primary/30" : "border-border text-muted-foreground"}`}
              >
                <Link
                  prefetch={false}
                  href={queryHref(path, next)}
                  aria-current={selected.sort === sort ? "true" : undefined}
                  onClick={(event) => selectTag(event, next)}
                >
                  {t(sort === "name-asc" ? "ascending" : "descending")}
                </Link>
              </Button>
            );
          })}
