...
export default async function LocaleLayout({children, params}: {children: React.ReactNode; params: Promise<{locale: string}>}) {
  ...
  return <NextIntlClientProvider locale={locale} messages={{UI:messages.UI}}><LocaleDocument locale={locale}/>
    <a href="#main" className="sr-only z-50 rounded-md bg-primary p-3 text-white focus:not-sr-only focus:fixed focus:top-2 focus:left-2">{t('skip')}</a>
    ...
    <main id="main" className="scroll-smooth scroll-mt-18 mx-auto mt-18 min-h-[75vh] max-w-[1480px] px-4 py-8 sm:px-8 lg:px-10">{children}</main>
    <footer className="border-t border-border">
      <div className="mx-auto flex max-w-[1480px] flex-wrap justify-between gap-3 px-6 py-7 text-xs text-muted-foreground">
        ...
        <a href="/NOTICE.md" className="underline underline-offset-4">{t('attribution')}</a>
      </div>
    </footer>
